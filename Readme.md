...
$$
A_t^{\text{discrete}} = \{0, 1, ..., n-1\} \times \{0, 1, 2\}
$$

- 节点选择：$\arg\min_i \text{cost}_i \text{ s.t. } \text{capacity}_i \geq \text{requirement}$
- 资源等级：{低配，标准，高配}

**连续动作空间**（Policy Gradient/A2C）：

$$
A_t^{\text{continuous}} = [\alpha_1, \alpha_2, ..., \alpha_n] \in [0, 1]^n
$$

- $\alpha_i$: 分配到节点 $i$ 的请求比例

**多 Agent 动作**（QMIX）：

$$
A_t^{\text{multi}} = (a_1, a_2, ..., a_m), \quad a_i \in A_{\text{local}}
$$
